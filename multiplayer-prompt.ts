namespace game {
    ...
    //% answerLength.max=24
    //% group="Prompt"
    export function askPlayerForString(player: number, message: any, answerLength = 12) {
        if (player < 1 || player > 4 || player % 1 != 0) {
            throw "Player must be an integer between 1 and 4."
        }
        let t: game.PromptTheme = {
            colorPrompt: 1,
            colorInput: 3,
        ...
        let p = new game.MultiplayerPrompt(player, t);
    ...
    export class MultiplayerPrompt {
        theme: PromptTheme;
